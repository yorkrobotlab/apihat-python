...
layout = html.Div([
    create_div('Motor Control',[
        html.Div([
            html.Div([
                html.H6("Motor 1"),
                daq.Slider(id="yrl031-motor1-slider",value=0,min=-127,max=127,handleLabel={"showCurrentValue":True,"label":"SPEED"}),
            ]),
            html.Div([
                html.H6("Motor 2"),
                daq.Slider(id="yrl031-motor2-slider",value=0,min=-127,max=127,handleLabel={"showCurrentValue":True,"label":"SPEED"}),
            ]),
        ],style={"width":"40%"}),
        html.Div([
            html.Div([html.Div(style=button_style),html.Button('Forwards', id='yrl031-forwards-button', style=button_style),html.Div(style=button_style)],style=flex_style),
            html.Div([
                html.Button('Left Turn', id='yrl031-left-button',style=button_style),
                html.Button('Stop', id='yrl031-stop-button',style=button_style),
                html.Button('Right Turn', id='yrl031-right-button',style=button_style),
            ]),
            html.Div([html.Div(style=button_style),html.Button('Backwards', id='yrl031-backwards-button', style=button_style),html.Div(style=button_style)],style=flex_style),
        ], style = {"width":"49%"}),
        html.Div([daq.Slider(id="yrl031-motor-speed-slider",vertical=True,value=20,min=0,max=127,size=130,updatemode='drag',handleLabel={"showCurrentValue":True,"label":"SPEED"}),]),

    ]),
    html.Div(id='yrl031-encoder-div',style=border_style),
    html.Div(id='yrl031-motor-output-div'),
    dcc.Interval(
            id='yrl031-interval-component',
...
@app.callback(
    Output('yrl031-motor1-slider','value'),
    [Input('yrl031-memory','modified_timestamp')],
    [State('yrl031-memory','data')])
def on_data(ts,data):
...
@app.callback(
    Output('yrl031-motor2-slider','value'),
    [Input('yrl031-memory','modified_timestamp')],
    [State('yrl031-memory','data')])
def on_data(ts,data):
    if ts is None: raise PreventUpdate
...
@app.callback(
    Output('yrl031-motor-output-div', 'children'),
    [Input('yrl031-motor1-slider', 'value'), Input('yrl031-motor2-slider','value')])
def motor_speed_callback(m1,m2):
    #motors.set_motor_speeds(m1/100.0,m2/100.0)
    yrl031.set_motor_speeds(int(m1),int(m2))
    return(html.P("Setting motor 1 to %d, motor 2 to %d" % (m1,m2)))

@app.callback(
    Output('yrl031-memory','data'),
    [Input('yrl031-forwards-button','n_clicks'),
    Input('yrl031-backwards-button','n_clicks'),
    Input('yrl031-left-button','n_clicks'),
    Input('yrl031-right-button','n_clicks'),
    Input('yrl031-stop-button','n_clicks'),
    ],
    [State('yrl031-memory','data'),
    State('yrl031-motor-speed-slider','value')])
...
@app.callback(
    Output('yrl031-encoder-div','children'),    [Input('yrl031-interval-component', 'n_intervals')])
